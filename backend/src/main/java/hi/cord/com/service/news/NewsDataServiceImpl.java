...
import hi.cord.com.domain.news.NewsWebSite;
import hi.cord.com.domain.sequence.CustomSequence;
import hi.cord.com.service.nlp.NlpPhraseService;
...
public class NewsDataServiceImpl implements NewsDataService {
    ...
    @Override
    @Caching(cacheable = {@Cacheable(key = "'news'+#pk", value = "news")})
    public NewsData findById(String pk) {
        return newsDataRepository.findByPk(pk);
    }
    ...
    public void update(NewsData newsData) {
        NewsData dbNews = this.findById(newsData.getPk());
        if (dbNews != null)
        ...
        Thread thread = new Thread(() -> {
            ...
            while (isRight) {
                ...
                try {
                    ...
                    newsData.setPk(webSiteName + "_" + i);
                    newsData.setIdx(i);
