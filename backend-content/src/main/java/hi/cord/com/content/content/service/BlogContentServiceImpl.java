...
import hi.cord.com.content.comment.domain.Comment;
import hi.cord.com.content.comment.domain.CommentRepository;
import hi.cord.com.content.content.domain.BlogContent;
import hi.cord.com.content.content.domain.BlogContentRepository;
import hi.cord.com.content.file.domain.FileData;
import hi.cord.com.content.file.domain.FileDataRepository;
...
public class BlogContentServiceImpl implements BlogContentService {
    ...
    public BlogContent findByIdx(long idx, String nickname) {
        BlogContent blogContent = blogContentRepository.findByIdxAndCreatedByEntityNickname(idx, nickname);
        if(blogContent != null){
            List<FileData> fileList = fileDataRepository.findAll();
            List<Comment> commentList = commentRepository.findAll();
            if (fileList != null) {
    ...
    public boolean deleteById(long idx, String accessBy) {
        BlogContent dbBlogContent = blogContentRepository.findByIdxAndCreatedByEntityNickname(idx, accessBy);
        if (dbBlogContent == null) {
            return false;
        }
    ...
    public long getIdxByNickname(String nickname) {
        BlogContent blogContent = blogContentRepository.findFirstByCreatedByEntityNicknameOrderByIdxDesc(nickname);
        if (blogContent == null) {
            return 1;
        }
